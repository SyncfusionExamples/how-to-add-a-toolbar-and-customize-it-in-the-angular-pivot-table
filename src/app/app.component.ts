...
import { IDataOptions, IDataSet, PivotViewModule, GroupingBarService,
         ToolbarItems,ToolbarService, PivotView, DataSourceSettings, DisplayOption,
         NumberFormattingService,ConditionalFormattingService,FieldListService
...
@Component({
  ...
  providers: [GroupingBarService,ToolbarService,NumberFormattingService,
...
export class AppComponent {
  ...
  beforeToolbarRender(args: any) {
    args.customToolbar.splice(12,0,{
      prefixIcon: 'e-tool-expand pv-icons',
      tooltipText: 'Expand/Collapse',
      click: this.toolbarClicked.bind(this)
    })
  }
  toolbarClicked(args: any){
    (this.pivotGridObj!.dataSourceSettings as DataSourceSettings).expandAll 
...
renameReport(args: any) {
  ...
  if (args.reportName === item.reportName) { item.reportName = args.rename; } }); 
  if (localStorage['pivotviewReports'] && localStorage['pivotviewReports'] !== "") {
      localStorage['pivotviewReports'] = JSON.stringify(reportCollection);
  }
}
  ...
  ngOnInit(): void {
    this.toolbarOptions = [
      'New', 'Save', 'SaveAs','Load','Remove','Rename','Chart','Grid','Export',
      'SubTotal','GrandTotal','Formatting','FieldList', 'Expand/Collapse'
    ] as ToolbarItems[];
    this.displayOption = {
